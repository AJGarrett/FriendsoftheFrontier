namespace UserControls
{
    ...
    public partial class PostList : UserControl
    {
        ...
        private void BindPosts()
        {
            ...
            var visiblePosts = new List<IPublishable>();

            if (System.Web.HttpContext.Current.User.Identity.IsAuthenticated)
                visiblePosts = this.Posts.FindAll(p => p.IsVisible);
            else
            {
                Category cat = Category.Categories.Find(delegate(Category c) { return c.Title == "Members Only"; });
                visiblePosts = this.Posts.FindAll(p => p.IsVisible).FindAll(p => !p.Categories.Contains(cat));
